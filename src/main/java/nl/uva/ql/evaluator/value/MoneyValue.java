...
public class MoneyValue extends Value {
	...
	public Value divideMoney(MoneyValue arg){
		return new MoneyValue(arg.getValue().divide(this.getValue(), 2, RoundingMode.HALF_UP));
		
	}
	
	@Override
	public Value divideInteger(IntegerValue arg){
		return new MoneyValue(new BigDecimal(arg.getValue()).divide(this.getValue(), 2, RoundingMode.HALF_UP));
	}
